...
def create_square_boundery(
    ...
    if thickness is None:
        descrete_thickness: int = 1
    else:
        descrete_thickness: int = xp.ceil(thickness / delta_x)
        realised_thickness: float = descrete_thickness * delta_x
    ...
    # create the boundery
    x_start = int(upper_left_corner[0] / delta_x)
    y_start = int(upper_left_corner[1] / delta_x)
    x_end = int((upper_left_corner[0] + x_size) / delta_x)
    y_end = int((upper_left_corner[1] + y_size) / delta_x)
    boundery[x_start:x_end, y_start:y_end] = value
    # add the thickness
    boundery[
        x_start + descrete_thickness : x_end - descrete_thickness,
        y_start + descrete_thickness : y_end - descrete_thickness,
    ] = default_value
