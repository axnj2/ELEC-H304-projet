...
beta_wall = 2 * np.pi * FREQ_REF / speed_of_light_in_wall  # propagation constant in the wall
...
def get_total_transmission_coeff(theta_i:float, theta_t:float, s:float) -> float:
    ...
    denom = np.abs(1 - reflection_coeff * np.exp(-2j * beta_wall * s) * np.exp(1j*beta*2*s*np.sin(theta_t)*np.sin(theta_i)))
...
print(f"total transmission coefficient : {get_total_transmission_coeff(0, 0, WALL_THICKNESS)}")
add_detector(
    ax,
    DETECTOR_1_POS,
    "red",
...
add_detector(ax, DETECTOR_1_POS, "blue", E_amp, distance_func=distance_from_source, name="free space")
real_distance = np.sqrt((DETECTOR_1_POS[0] - SOURCE_POS[0]) ** 2 + (DETECTOR_1_POS[1] - SOURCE_POS[1]) ** 2)
print(f"predicted electric field amplitude at detector 1 : {np.abs(get_total_transmission_coeff(0, 0, WALL_THICKNESS)) * Z_0 *np.sqrt(beta/(8*np.pi)) * TOTAL_CURRENT/np.sqrt(real_distance)}")
print(f"real electric field amplitude at detector 1 : {E_amp[round(DETECTOR_1_POS[1]/DELTA_X), round(DETECTOR_1_POS[0]/DELTA_X)]}")
